...
class BrouteIdentifyNo(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "BrouteIdentifyNo":
        if len(data) != 16:
            raise ValueError(f"Invalid data length: expected 16 bytes, got {len(data)}")
...
class OneMinuteCumulativeEnergy(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "OneMinuteCumulativeEnergy":
        if len(data) != 15:
            raise ValueError(f"Invalid data length: expected 15 bytes, got {len(data)}")
...
class Coefficient(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "Coefficient":
        if len(data) != 4:
            raise ValueError(f"Invalid data length: expected 4 bytes, got {len(data)}")
...
class CumulativeEnergySignificantDigit(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeEnergySignificantDigit":
        if len(data) != 1:
...
class CumulativeEnergyMeasurement(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeEnergyMeasurement":
        if len(data) != 4:
            raise ValueError(f"Invalid data length: expected 4 bytes, got {len(data)}")
...
class CumulativeEnergyUnit(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeEnergyUnit":
        if len(data) != 1:
            raise ValueError(f"Invalid data length: expected 1 byte, got {len(data)}")
...
class CumulativeEnergyMeasurementHistory1(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeEnergyMeasurementHistory1":
        if len(data) != 194:
            raise ValueError(
...
@dataclass
class CumulativeEnergyMeasurementHistory1NormalDir(CumulativeEnergyMeasurementHistory1):
...
class CumulativeHistoryCollectDay1(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeHistoryCollectDay1":
        if len(data) != 1:
            raise ValueError(f"Invalid data length: expected 1 byte, got {len(data)}")
...
class IntCumulativeEnergyMeasurement(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "IntCumulativeEnergyMeasurement":
        if len(data) != 11:
            raise ValueError(f"Invalid data length: expected 11 bytes, got {len(data)}")
        year, month, day, hour, minute, second, power = struct.unpack(">HBBBBBI", data)
        timestamp = datetime(year, month, day, hour, minute, second)
...
class CumulativeEnergyMeasurementHistory2(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeEnergyMeasurementHistory2":
        ...
        if (
            ...
            return cls(timestamp=None, record_count=1, energy_records=[(None, None)])
        ...
        for _ in range(record_count):
            if offset + 8 > len(data):
                raise ValueError("Insufficient data for the expected number of records")
...
class CumulativeHistoryCollectDay2(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeHistoryCollectDay2":
        if len(data) != 7:
            raise ValueError(f"Invalid data length: expected 7 bytes, got {len(data)}")

        year, month, day, hour, minute, collect_count = struct.unpack(">HBBBBB", data)
        if (
            year == 0xFFFF
            and month == 0xFF
            and day == 0xFF
...
class CumulativeEnergyMeasurementHistory3(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeEnergyMeasurementHistory3":
        if len(data) < 7 or (len(data) - 7) % 8 != 0:
            raise ValueError(
                f"Invalid data length: expected at least 7 bytes with 8-byte multiples, got {len(data)}"
            )
...
class CumulativeHistoryCollectDay3(Property):
    ...
    @classmethod
    def decode(cls, data: bytes) -> "CumulativeHistoryCollectDay3":
        if len(data) != 7:
            raise ValueError(f"Invalid data length: expected 7 bytes, got {len(data)}")
